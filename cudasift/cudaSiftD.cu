    ...
    for (int dy = 0; dy < SCALEDOWN_H + 4; dy += 5) {
        {
            ...
            if (tx<maxtx) {
                brow[tx4] = k0 * (inrow[2 * tx] + inrow[2 * tx + 4]) + 
                    k1 * (inrow[2 * tx + 1]+inrow[2 * tx + 3]) + k2 * inrow[2 * tx + 2];
                if (dy >= 4 && !(dy & 1)) {
                    d_Result[yWrite[dy + 0] + xWrite] = k2 * brow[tx2] + 
                        k0 * (brow[tx0] + brow[tx4]) + k1 * (brow[tx1]+brow[tx3]);
                }
            }
            __syncthreads();
        }
            ...
            if (tx < maxtx) {
                ...
                if (dy >= 3 && (dy & 1)) {
                    d_Result[yWrite[dy + 1] + xWrite] = k2 * brow[tx3] + 
                        k0 * (brow[tx1] + brow[tx0]) + k1 * (brow[tx2] + brow[tx4]);
                }
            }
            __syncthreads();
        }
            ...
            if (tx<maxtx) {
                ...
                if (dy >= 2 && !(dy & 1)) {
                    d_Result[yWrite[dy + 2] + xWrite] = k2 * brow[tx4] + 
                        k0 * (brow[tx2] + brow[tx1]) + k1 * (brow[tx3] + brow[tx0]);
                }
            }
            __syncthreads();
        }
            ...
            if (tx < maxtx) {
                ...
                if (dy >= 1 && (dy & 1)) {
                    d_Result[yWrite[dy + 3] + xWrite] = k2 * brow[tx0] + 
                        k0 * (brow[tx3] + brow[tx2]) + k1 * (brow[tx4] + brow[tx1]);
                }
            }
            __syncthreads();
        }
            ...
            if (tx < dx2 && xWrite < width / 2) {
                ...
                if (!(dy & 1)) {
                    d_Result[yWrite[dy + 4] + xWrite] = k2 * brow[tx1] + 
                        k0 * (brow[tx4] + brow[tx3]) + k1 * (brow[tx0] + brow[tx2]);
                }
            }
            __syncthreads();
        }
